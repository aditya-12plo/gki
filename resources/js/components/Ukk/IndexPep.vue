...
<div class="breadcrumb"> 
  <div class="panel">
    <h1 class="animated fadeInLeft" align="center">Dokumen PEPS</h1>                
  </div>
</div>
...
<div class="container-fluid display-page" id="display-post-category" >
 <!-- @create Modal--->
        ...
        <template slot="header" ><h4 align="center">Form Penambahan Dokumen PEPs</h4></template>
        <template slot="body" >
...
<div class="form-group" style="margin-top:40px !important;">
    ...
    <div class="invalid-tooltip" v-for="error of errors['dokumen']">
        {{ error }}
    </div>
...
<div class="form-group" style="margin-top:40px !important;">
    <label>Nama Dokumen</label> 
    <input type="file" name="file" id="file" v-on:change="uploadFile" class="form-control"> 
    <div class="invalid-tooltip" v-for="error of errors['dokumen']">
        {{ error }}
    </div>
    <br>
    <input class="submit btn btn-primary" @click.prevent="downloadPdfItem(forms,1)" type="button" value="Download">
    <p class="center-block">* Type dokumen .pdf And Max 10 MB</p>    
...
import EditDeleteDownloadActions from '../Buttons/EditDeleteDownloadActions.vue'  
 

Vue.component('edit-delete-download-actions', EditDeleteDownloadActions)
  ...
  data () {
    return {  
        ...
        fields: [ 
            ...
            {
                name: '__component:edit-delete-download-actions',
                title: 'Actions',
                titleClass: 'text-center',
                dataClass: 'text-center'
            }
        ...
        methods: {
            ...
            updateData() { 
                ...
                axios.post('/ukk-peps-update/'+this.forms.id, masuk).then(response => {
                    if(!response.data){ 
            ...
            submitData() { 
                ...
                axios.post('/ukk-peps', masuk).then(response => {
                    if(!response.data){ 
                        window.location.href = window.webURL; 
                    }else{ 
                         if(response.data.status == 200){   
           ...
           deleteItem(item ,index = this.indexOf(item)){
            ...
            downloadPdfItem(item ,index = this.indexOf(item)){
                ...
                axios({
                url: '/ukk-peps-download-file-pdf',
                method: 'POST',
                data: masuk,
            ...
            fileDownload (value) {
                return '<a href="'+this.url+value+'" target="_blank"> <input class="submit btn btn-primary" type="button" value="Download"> </a>';
            },
            ...
            onLoadingError() {
               this.isLoading = true;
               axios.get('/ukk-peps').then((response) => {
                   if(!response.data){ 
                        window.location.href = window.webURL; 
                    }else{ 
                        this.isLoading = false;
