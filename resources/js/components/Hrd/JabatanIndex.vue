...
<div class="breadcrumb"> 
  <div class="panel">
    <h1 class="animated fadeInLeft" align="center">LIST JABATAN KARYAWAN</h1>                
  </div>
</div>
                                ...
                                <div class="dataTables_length" id="zero_configuration_table_length">
                                    <div class="row row-xs">
                                        ...
                                        <div class="col-md-12">
                                            ...
                                            <input type="text" v-model="filterText" class="form-control form-control-sm" @keyup.enter="doFilter" placeholder="Nama Jabatan"> 
                                        </div>
                                        <div class="col-md-12">
                                            <br>
                                            <button type="button" class="btn btn-raised btn-raised-info m-1"  @click="createItem()"><i class="fa fa-plus"></i> Jabatan</button>
...
<div class="container-fluid display-page" id="display-post-category" >
 <!-- @create Modal--->
        ...
        <template slot="header" ><h4 align="center">Penambahan Jabatan</h4></template>
        <template slot="body" >
...
<div class="form-group form-animate-text" style="margin-top:40px !important;">
    ...
    <input type="text" class="form-control" v-model="forms.jabatan" required="" aria-required="true">
...
<!-- @view --->
        ...
        <template slot="header" ><h4 align="center">Detail Jabatan</h4></template>
        <template slot="body" >
                <div class="modal-body">
...
<div class="form-group form-animate-text" style="margin-top:40px !important;">
    <label>Nama Jabatan</label>
    <h5 class="media-heading"><b>{{forms.jabatan}}</b></h5>  
</div>    
...
<!-- @update --->
        ...
        <template slot="header" ><h4>Edit Jabatan</h4></template>
        <template slot="body" >
...
export default {
  ...
  data () {
    return { 
        ...
        forms:new CrudForm({index:'',  id:'' , jabatan:'', created_at:''}),
        ...
        methods: {
            ...
            updateData() { 
                axios.put('/jabatan/'+this.forms.id, this.forms).then(response => {
                    if(!response.data){ 
            ...
            submitData() { 
                axios.post('/jabatan', this.forms).then(response => {
                    if(!response.data){ 
                        window.location.href = window.webURL; 
                    }else{ 
                         if(response.data.status == 200){   
                    ...
                    this.$swal({
                        ...
                        if (result.value) {
                            var join_selected_values = ttl.join(","); 
                                axios.delete('/jabatan/delete-all/'+ join_selected_values)
                                .then(response => {
                                    if(!response.data){ 
                                        window.location.href = window.webURL; 
                                    }else{
           ...
           deleteItem(item ,index = this.indexOf(item)){
            ...
            printItem(){
               this.isLoading = true;
               axios.get('/print-jabatan?filter='+this.filterText+'&min='+this.startTime.time+'&max='+this.endtime.time).then((response) => {
                   if(!response.data){ 
                        window.location.href = window.webURL; 
                    }else{ 
            ...
            downloadItem() {
               this.isLoading = true;
                var masuk = {
                        ...
                        'filename':'download-jabatan.xls'
                    }
                    axios({
                    url: '/download-excel-jabatan',
                    method: 'POST',
                    data: masuk,
                    ...
                    link.href = url;
                    link.setAttribute('download', 'download-jabatan.xls');
            ...
            onLoadingError() {
               this.isLoading = true;
               axios.get('/jabatan').then((response) => {
                   if(!response.data){ 
                        window.location.href = window.webURL; 
                    }else{ 
                        this.isLoading = false;
