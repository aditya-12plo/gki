...
<div class="breadcrumb"> 
  <div class="panel">
    <h1 class="animated fadeInLeft" align="center">ROLES</h1>                
  </div>
</div>
                                ...
                                <div class="dataTables_length" id="zero_configuration_table_length">
                                    <div class="row row-xs">
                                        ...
                                        <div class="col-md-12">
                                            ...
                                            <input type="text" v-model="filterText" class="form-control form-control-sm" @keyup.enter="doFilter" placeholder="Role Name"> 
                                        </div>
                                        <div class="col-md-12">
                                            <br>
                                            <button type="button" class="btn btn-raised btn-raised-info m-1"  @click="createItem()"><i class="fa fa-plus"></i> Role</button>
...
<div class="container-fluid display-page" id="display-post-category" >
 <!-- @create Modal--->
        ...
        <template slot="header" ><h4 align="center">Add New Role</h4></template>
        <template slot="body" >
...
<div class="form-group form-animate-text" style="margin-top:40px !important;">
    ...
    <input type="text" class="form-control" v-model="forms.role_code" required="" aria-required="true" @keydown.space="(event) => event.preventDefault()">
    <div class="invalid-tooltip" v-for="error of errors['role_code']">
        {{ error }}
    </div> 
</div>   
...
<div class="form-group form-animate-text" style="margin-top:40px !important;">
    ...
    <input type="text" class="form-control" v-model="forms.role_name" required="" aria-required="true">
    <div class="invalid-tooltip" v-for="error of errors['role_name']">
        {{ error }}
    </div>  
</div>            
...
<!-- @view --->
        ...
        <template slot="header" ><h4 align="center">Detail Role</h4></template>
        <template slot="body" >
                <div class="modal-body">
...
<div class="form-group form-animate-text" style="margin-top:40px !important;">
    <label>Role Code</label>
    <h5 class="media-heading"><b>{{forms.role_code}}</b></h5> 
</div>   

<div class="form-group form-animate-text" style="margin-top:40px !important;">
    <label>Role Name</label>
    <h5 class="media-heading"><b>{{forms.role_name}}</b></h5>  
</div>    
...
export default {
  ...
  data () {
    return {  
        ...
        forms:new CrudForm({index:'',  id:'' , role_code:'', role_name:'', created_at:''}),
        ...
        methods: {
            ...
            updateData() { 
                axios.put('/roles/'+this.forms.id, this.forms).then(response => {
                    if(!response.data){ 
            ...
            submitData() { 
                axios.post('/roles', this.forms).then(response => {
                    if(!response.data){ 
                        window.location.href = window.webURL; 
                    }else{ 
                         if(response.data.status == 200){   
                    ...
                    this.$swal({
                        ...
                        if (result.value) {
                            var join_selected_values = ttl.join(","); 
                                axios.delete('/role/delete-all/'+ join_selected_values)
                                .then(response => {
                                    if(!response.data){ 
                                        window.location.href = window.webURL; 
                                    }else{
